...
const styles = StyleSheet.create({
  ...
  contentContainerStyle: {
    // flexGrow: 1, //该设置很关键 开启时 当聊天内容不足Flatlist内容高度时 会在上方留出大片空白
...
> extends React.PureComponent<MessageContainerProps<TMessage>, State> {
  ...
  renderRow = ({ item, index }: ListRenderItemInfo<TMessage>) => {
    ...
    if (messages && user) {
      ...
      const messageProps: Message['props'] = {
        ...
        inverted,
        nextMessage,
        position: item.user.fromId === user.fromId ? 'right' : 'left',
      }
  ...
  onLayoutList = () => {
    ...
    // 解决当聊天内容不足Flatlist容器高度时 禁止其滚动
    if (Platform.OS === 'ios') {
      let height = e.nativeEvent.layout.height;
      let containerHeight = windowHeight - 44 - 50 - getStatusBarHeight(true) - getBottomSpace();
